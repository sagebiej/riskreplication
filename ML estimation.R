...
## Prepare data for model estimation with ML: One obsrevation per choice 

ml_data <- genDesign(respondents = nrow(allcountries)) %>%      # Use genDesign function to create design
...
write_dta(ml_data, "replication_ml.dta")  ## save as dta to use in Stata


## Prepare objects to store estimation results
...
## Loop over countries and estimate 6 models EU, EX, PT with and without covariates
...
## save models
save(list=(ls(pattern = "mlmodels_")), file = "storedResults/mlmodels.RData")
